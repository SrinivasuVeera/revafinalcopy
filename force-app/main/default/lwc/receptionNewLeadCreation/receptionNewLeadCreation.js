...
import recsubmitApplication from '@salesforce/apex/NewLeadCreation.recsubmitApplication';
...
onProgramChange(e) { this.selectedProgram = e.target.value; }
    ...
    try {
        ...
        /*let mobile = this.mobileNumber.replace(/\D/g, ''); 
        let last10 = mobile.slice(-10);
        let formattedMobile = this.phoneCode.includes('+91') ? '+91-' + last10 : mobile;
        console.log('RAW MOBILE FROM INPUT:', this.mobileNumber);
        console.log('FINAL FORMATTED MOBILE => ', formattedMobile);

        console.log('STEP 1: Updating Lead Record...'); */
...
if (!this.leadId) {
    ...
    if (validationResult === 'EMAIL_MOBILE_EXISTS') {
        ...
        const result = await recsubmitApplication({ studentData });
